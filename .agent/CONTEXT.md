...
| 周/月频重采样（S5） | PRD「日频为主、周频与月频兼容」的引擎落地：引擎 resampleToFrequency 纯函数（周频按 ISO 周周一分桶、月频按日历月前缀，桶值取期末观测值，日期标签保留期末真实日期；日频原样）；prepareDataset 可选 frequency 输入，在派生变换之前重采样 → 周/月收益率自然为期末价口径；编排透传 config.frequency；参考/检验期、滚动窗口、事件标签、分箱沿重采样后轴自然继承（事件日若非期末日则沿既有 skipped 机制跳过）。 |
...
| 滚动并行化（P1） | PRD「滚动窗口与置换检验应支持后台并行化」落地：引擎三接缝 planRollingJobs（配对×窗口×方法同序枚举，任务纯数据可结构化克隆）/ executeRollingJob（单任务，退化返回 skipped 不抛错）/ reassembleRollingResults（按任务索引重组 → 任意并发度输出与串行一致，确定性可复现），rollingWindowTests 重构为经接缝实现；API 层 node:worker_threads 固定池（并发上限 min(availableParallelism,4,任务数)，单任务失败整体拒绝并清理）；编排经 RunnerDeps.rollingExecutor? 注入（缺省同线程串行保单测纯净），生产组合根 makeRunnerDeps 注入 createParallelRollingExecutor；worker 入口 rolling-worker.ts 由 tsup 第三入口独立打包，生产用 .js（存在性检查）开发回退 .ts。 |
...
| 批注收藏分享（X4） | PRD L140/L356 右栏「研究注释、收藏、导出与分享」。批注：task_annotations 表（迁移 007，随任务级联删除，task_id+workspace_id 双键作用域）+ /tasks/:id/annotations GET/POST/DELETE（内容 trim 后 1~2000 字）；收藏：tasks.favorited 旗标 + PUT /tasks/:id/favorite（taskRecord 回显，历史页星标）；分享：前端复制结果页 URL（单实例匿名工作区不落库，团队协作分享链接归 V2）。 |
| 顶部导航（X5） | PRD L330-338 信息架构七项：工作区/新建分析/数据集/分析结果/报告/历史任务/设置。新增三页：/workspace 工作区总览（任务/文件/模板计数 + 功能入口，纯前端聚合既有列表 API）；/datasets 数据集管理（已上传 CSV 列表 + 删除，后端补 DELETE /api/files/:id 工作区作用域，无外键引用，删除确认弹窗提示旧任务重跑风险）；/reports 报告（已完成任务列表 + 一键导出 15_full_report.html，复用 export-report 客户端纯函数）。/results 无参时为已完成任务索引直达；导航高亮按路径前缀归属（/results/:taskId 归分析结果）。 |
| LLM 模板 A/B（X6） | PRD L648 提示词模板多版本对照。多版本资产策略：v1=根目录文件（meta.json files 声明，向后兼容）；v2=prompts/v2/ 子目录固定文件名（结论先行/要点式/证据分级变体，占位符 12 字段集与 v1 一致）；meta.json versions 数组声明合法域。契约 promptVersion 枚举化 ['v1','v2']；透传链 runAnalysis → deps.interpret(…, promptVersion) → interpretContext → loadPromptAssets(version)，未知版本快速失败抛错。前端：向导研究问题下方版本选择；结果页「A/B 对比：换用另一版本重跑」复用 X2 克隆机制（/?clone=&prompt= 覆盖，差异经失效提示自动暴露）+ 配置摘要提示词版本行。 |
...
| 任务运行编排（runAnalysis） | T17-A domain 层纯 DI：数据加载（ticker 适配器/CSV 映射 date_col/close_col/adj_close_col）→ prepareDataset → 卡方独立性+拟合优度（GOF，S1）+连续注册表四法 → 滞后扫描（maxLag>0 时）→ 校正（全样本按族分批、GOF/滞后/滚动各自单独成批）→ 滚动窗口 → 每源审计 → buildLlmContext → interpret；重跑语义为 result/audit 整体替换、llm_artifacts ON CONFLICT 更新；P2 可选 onProgress 上报编排 10 步进度（0..9，与 RUN_STEPS 标签表对齐，持久化步由路由层承担）。 |
| 异步任务与进度展示（P2/X1） | PRD L137/L556 落地：POST /tasks/:id/run 202 受理即返回，后台异步执行（成功 completed / 失败回写 failed+errorMessage）；域层 run-progress 单实例内存注册表（RUN_STEPS 11 步标签：加载数据…结果持久化，终态清空）；GET /tasks/:id/progress 返回 {status, progress}（归属校验）；启动清扫 recoverInterrupted 把重启中断的 running 任务置 failed 注明可重试；前端结果页运行中 2s 轮询状态 + 1s 轮询进度（进度条 + 第 x/y 步），失败页一键重试，排队中展示等待文案；单实例部署下队列状态仅状态机语义（无多实例队列）。 |
| 运行/结果端点 | POST /api/tasks/:id/run（P2：202 受理后台异步执行，运行中重复提交 409）与 GET /api/tasks/:id/progress（运行进度，终态后 progress=null）与 GET /api/tasks/:id/results（task+results+audit+panel+llm 四产物一次返回，出参过 Zod 校验）。 |
| 新建分析向导（HomePage） | T17-B 五步 Steps：数据源（ticker/CSV 动态列表+列映射）→ 样本区间 → 期间划分 → 检验选项 → 预览与运行；提交前 taskConfigSchema.safeParse 全量校验（workspaceId 占位，服务端 Cookie 覆盖），创建→202 受理异步运行（P2）→跳结果页跟踪进度（X1）。 |
...
| 部署拓扑（T20） | Render Blueprint 单 web 服务同源托管 API + web 产物（SameSite=Lax 工作区 Cookie 跨站会被当第三方 Cookie 拦截，同源免除 CORS）；api 经 tsup 三入口打包（index + infrastructure/migrate + infrastructure/rolling-worker，生产不再依赖 tsx），启动前 node dist/infrastructure/migrate.js 幂等迁移；prompts/ 与 infra/db/migrations 不入 bundle，prompt-assets/migrate 自模块目录向上寻路 8 级；mountWebStatic 由 WEB_DIST_DIR 控制（未设→纯 API 模式），SPA 深链回退 index.html 且 /api 永不回退。 |

## 模块与接缝

| 模块 | 职责 | 关键接缝 |
| --- | --- | --- |
| packages/schemas | Zod 契约唯一来源（入参/出参/持久化双向校验）+ 配置差异与失效域分类纯函数（config-diff，X2）+ 批注契约（annotation，X4） | 被 api / analysis-engine / web 消费 |
...
| apps/web | React + Vite + AntD + tokens.css（禁 Tailwind）：顶部导航七项对齐（X5，含工作区总览/数据集管理/报告导出三新页，/results 无参为已完成任务索引）；新建分析向导（G6 模板载入/保存/复制分析预填；G11 派生序列编辑器；G12 CSV 第二文件双源审计；X2 克隆基线失效提示；X6 提示词版本选择）/三栏结果页（G10 七 Tab 含原始导出；P2/X1 运行中进度条轮询与失败重试；X2 调整参数并重跑入口；X4 研究注释卡+收藏星+复制分享链接；X6 A/B 对比重跑入口+配置摘要提示词版本行）/历史任务列表（复制分析 + 收藏星标）；G7 路由级 React.lazy 懒加载 + manualChunks（vendor-antd/vendor-react 独立缓存）；lib/api.ts fetch 封装（credentials:'include'，出参过 Zod 校验）+ lib/export.ts 下载原语（downloadText 支撑 md/html）+ lib/export-report.ts 01~15 编号导出生成器 | 经 /api 调网关；样式一律 tokens.css 变量与语义类，页面补充样式在 app.css（仅引用 Token 变量） |
| infra/db | PostgreSQL 免管理员部署脚本 + 迁移 SQL（001 tasks/result_rows/audit_rows、002 uploaded_files、003 llm_artifacts、004 lag 约束放宽至 ±60、005 run_panels 导出面板快照、006 config_templates 分析模板、007 task_annotations 批注 + tasks.favorited 收藏） | migrate.ts 运行器（schema_migrations 记账）；start/stop-postgres.ps1 必须 UTF-8 带 BOM（PowerShell 5.1 无 BOM 时中文注释破坏解析） |
| prompts/ | LLM 提示词模板（v1 根目录 + v2 子目录，meta.json versions 声明合法域，X6）+ output_schema.json（PRD 模块 K 10 字段声明，G8 已与 llmOutputSchema 对拍核验） | T16 经 loadPromptAssets(version) 消费（版本号写入 llm_trace）；未知版本快速失败；G8 loadOutputSchema 与 llmOutputSchema 双源一致性守卫测试 |
